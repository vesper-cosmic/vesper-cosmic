...
export async function sendOrderEmails(order, notionResult) {
  const customerResult = await safeSendEmail({
    to: order.email,
    replyTo: ownerEmail,
    subject: `${storeName} — Your order details have been received`,
  ...
  const ownerResult = await safeSendEmail({
    to: ownerEmail,
    replyTo: order.email,
    subject: `${storeName} — New order pending payment — ${order.fullName}`,
...
function customerEmailText(order) {
  const deliveryText = order.requiresShipping
    ? `After payment is completed, your handcrafted item will be prepared and shipped within ${order.fulfillmentTime}.\nYou'll receive a tracking number once it ships.`
...
Thank you for placing an order with Vesper Cosmic. Your order details have been received.

Order ID: ${order.orderId}
Product: ${order.productName}
Amount: $${order.price} USD
Payment status: Pending PayPal payment

${deliveryText}

If you have already completed payment, no further action is needed. I will review your order and be in touch within 24 hours to confirm the details.
...
function ownerEmailText(order, notionResult) {
  const notionStatus = notionResult?.error
    ? `Notion status: Failed — ${notionResult.error}`
    : "Notion status: Created successfully";

  return `${storeName}

New order pending PayPal payment
