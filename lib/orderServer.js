...
import { calculateShippingFee, getShippingTier } from "@/lib/shipping";
...
export async function validateMultiCheckoutPayload(payload) {
  ...
  for (let index = 0; index < items.length; index += 1) {
    ...
    if (!product) {
      ...
      normalizedItems.push({
        ...
        requiresShipping: Boolean(product.requiresShipping),
        requiresNailDetails: Boolean(product.requiresNailDetails),
        shippingTier: getShippingTier(product),
        quantity,
  ...
  // Calculate shipping fee based on the cart contents.
  const shipping = calculateShippingFee(
    normalizedItems.map((item) => ({
      product: { shippingTier: item.shippingTier },
      price: item.price,
...
/**
 * Look up a pending order in the in-memory store by orderId.
 * Used by the payment-confirmation routes to verify an order was
 * actually created before marking it as paid.
 */
export function findStoredOrder(orderId) {
  return storedOrders.find((item) => item.orderId === orderId) || null;
}
...
export async function sendOrderEmails(order, notionResult) {
  const customerEmailsEnabled =
    process.env.RESEND_CUSTOMER_EMAILS_ENABLED === "true";

  const customerResult = customerEmailsEnabled
    ? await safeSendEmail({
        to: order.email,
        replyTo: ownerEmail,
    ...
    : {
        skipped: true,
        reason:
          "Customer email disabled until a domain is verified in Resend. Set RESEND_CUSTOMER_EMAILS_ENABLED=true to enable.",
      };
