...
export function validateOrderPayload(payload) {
  ...
  if (product?.intentionType === "single") {
    const readyIntention = String(payload.readyIntention || "").trim();
    if (!readyIntention) {
      errors.readyIntention = "Select one intention for this piece.";
    } else if (!product.availableIntentions?.includes(readyIntention)) {
      errors.readyIntention = "Selected intention is invalid.";
    }
  }
  ...
  const order = {
    ...
    readyIntention: String(payload.readyIntention || "").trim(),
...
export function validateMultiCheckoutPayload(payload) {
  const errors = {};
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  ...
  items.forEach((raw, index) => {
    ...
    if (!product) {
      ...
      if (
        ...
        !String(details.readyIntention || "").trim()
      ) {
        itemErrors.readyIntention = "Select one intention for this piece.";
      }
      ...
      if (product.requiresBirthData) {
        ...
        if (product.formType === "C" && !String(birth.digitalCuriosityArea || "").trim()) {
          itemErrors.digitalCuriosityArea = "Select the area you are most curious about.";
        }
        if (product.formType !== "C" && !String(birth.baziIntention || "").trim()) {
          itemErrors.baziIntention = "Select what you are hoping to achieve.";
        }
      }
      ...
      normalizedItems.push({
        ...
        requiresNailDetails: Boolean(product.requiresNailDetails),
        quantity,
        intention: String(details.readyIntention || "").trim(),
...
export function normalizeConfirmedOrderPayload(payload) {
...
function notionOrderProperties(order, titlePropertyName) {
  ...
  if (order.baziIntention || order.readyIntention) {
    properties.客人意圖 = select(order.baziIntention || order.readyIntention);
  }
    ...
    客人意圖: {
      select: {
        options: [
          ...
          option("Health & Energy"),
          option("Wealth & Abundance"),
...
export function customerEmailText(order) {
  ...
  if (order.readyIntention) detailLines.push(["Focus", order.readyIntention]);
  ...
  if (order.baziIntention) detailLines.push(["Intention", order.baziIntention]);
  if (order.digitalCuriosityArea) {
    detailLines.push(["Curiosity area", order.digitalCuriosityArea]);
  }
...
Intention: ${order.baziIntention || order.digitalCuriosityArea || "N/A"}
Ready-made focus: ${order.readyIntention || "N/A"}
...
function sanitizeBirth(birth = {}) {
  return {
    ...
    baziIntention: String(birth.baziIntention || "").trim(),
    digitalCuriosityArea: String(birth.digitalCuriosityArea || "").trim(),
...
function formatNotes(order) {
  return [
    ...
    order.readyIntention ? `Ready-made focus: ${order.readyIntention}` : "",
