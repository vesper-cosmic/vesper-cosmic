...
export async function createNotionOrder(order) {
  ...
  return notion.pages.create({
    ...
    properties: notionOrderProperties(order),
  });
}
...
export async function setupNotionDatabase() {
  ...
  const databaseResult = await notion.databases.update({
  ...
  return { databaseResult, viewResult };
}

function notionOrderProperties(order) {
  const properties = {
    "Order ID": title(order.orderId),
...
async function safeSendEmail({ to, subject, text }) {
  try {
    ...
    return resend.emails.send({
      from: `Vesper Cosmic <${process.env.RESEND_FROM_EMAIL || ownerEmail}>`,
      to,
      subject,
      text,
    });
